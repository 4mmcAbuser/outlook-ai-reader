...
async function generateDraft(instruction, audioObj) {
    ...
    // Αυστηρή προσθήκη εντολής στο prompt ώστε το AI να ΜΗΝ απαντάει με "..." αν το context είναι κενό
...
ΑΠΑΓΟΡΕΥΕΤΑΙ ΡΗΤΑ να απαντήσεις με αποσιωπητικά "..." ή κενά placeholders. Αν το Email Thread είναι κενό ή ελλιπές, δημιούργησε μια αυτόνομη, ευγενική και ολοκληρωμένη απάντηση βασισμένη αποκλειστικά στην οδηγία του χρήστη.
...
{
 "intent": "draft",
 "content": "Το ολοκληρωμένο περιεχόμενο της απάντησης εδώ (Όχι placeholders, όχι αποσιωπητικά)."
}
(Το πεδίο intent μπορεί να είναι είτε "draft" είτε "question")`;
    ...
    try {
        ...
        const res = await fetch(url, {
            ...
            body: JSON.stringify({
                contents: [{ parts }],
                generationConfig: { temperature: 0.3 } // Μικρή αύξηση temperature για αποφυγή επαναλαμβανόμενων placeholders
            })
        });
        ...
        if (parsed.content === "...") {
            throw new Error("Το μοντέλο απέτυχε να συνθέσει κείμενο λόγω έλλειψης δεδομένων.");
        }
        ...
        voiceStatus.innerText = '❌ Σφάλμα AI: ' + e.message;
    }
}
