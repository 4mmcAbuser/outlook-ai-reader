    ...
    if (info.host === Office.HostType.Outlook) {
        ...
        Office.context.mailbox.addHandlerAsync(
            ...
            () => {
                ...
                // Αυξάνουμε το timeout σε 1000ms για το Outlook Live (CORS / Latency protection)
...
// Safe JSON parser that extracts correctly paired brackets and ignores trailing garbage
function safeJsonParse(rawStr) {
    ...
    if (start === -1) throw new Error("Δεν βρέθηκε αρχικό άγκιστρο '{'.");
    ...
    if (!jsonMatch) throw new Error("Δεν εντοπίστηκε έγκυρη δομή JSON.");
...
function initOutlookData() {
    ...
    // Καθαρισμός προηγούμενου context για αποφυγή cross-contamination
    emailContext.text = '';
    ...
    getFullConversationViaREST()
        ...
        .catch(() => {
            // Αν αποτύχει το REST API (συχνό στο Live.com consumer), πάμε στο fallback με retry σύστημα
            fallbackCurrentMail(0);
        });
}

// 🔄 SMART RETRY SYSTEM FOR OUTLOOK LIVE.COM CONTEXT ACQUISITION
function fallbackCurrentMail(retryCount = 0) {
    ...
    item.body.getAsync(Office.CoercionType.Text, (result) => {
        if (result.status === Office.AsyncResultStatus.Succeeded && result.value && result.value.trim().length > 0) {
            ...
            // Αν το email text επιστρέψει κενό, ξαναπροσπαθούμε μετά από 500ms (Max 3 retries)
            if (retryCount < 3) {
...
${emailContext.text.substring(0, 8000)}`;
    ...
    try {
        ...
        if (!resObj.summary || resObj.summary === "...") {
            throw new Error("Το AI επέστρεψε κενά placeholders (άδειο email context).");
        }
...
// ----------------------
// ----------------------
// GENERATE DRAFT WITH AGENTIC MEMORY & LATENCY COOLDOWN
// ----------------------
async function generateDraft(instruction, audioObj) {
    ...
    // 1. ΔΙΚΛΕΙΔΑ ΑΣΦΑΛΕΙΑΣ: Αν το email δεν έχει φορτώσει ακόμα, σταματάμε την εκτέλεση 
    // για να μην στείλουμε άδειο context στο AI και μας απαντήσει με "..."
    if (!emailContext.text || emailContext.text.trim() === "" || emailContext.text.includes("Σφάλμα ανάγνωσης.")) {
        ...
        document.getElementById('voiceStatus').innerText = "⏳ Το email φορτώνει ακόμα... Παρακαλώ ξαναπροσπαθήστε σε 1 δευτερόλεπτο.";
        return;
    }

    document.getElementById('voiceStatus').innerText = '🤖 Σκέφτομαι...';

    // Ανασύνταξη του prompt: Αφαιρέθηκαν τα παραδείγματα με σχόλια που μπέρδευαν το Gemma
    const systemPrompt = `Είσαι ένας Executive AI Assistant για επαγγελματική αλληλογραφία. Η απάντησή σου πρέπει να αποτελείται ΑΠΟΚΛΕΙΣΤΙΚΑ από ένα έγκυρο αντικείμενο JSON, χωρίς markdown ή επεξηγήσεις γύρω από αυτό.
ΑΠΑΓΟΡΕΥΕΤΑΙ ΑΥΣΤΗΡΑ να χρησιμοποιήσεις αποσιωπητικά (...) ή κενά templates. Γράψε ένα πλήρες, έτοιμο και ολοκληρωμένο κείμενο.
...
🎯 ΚΑΘΗΚΟΝ:
Επίστρεψε ένα αντικείμενο JSON με αυτήν ακριβώς τη δομή:
{
 "intent": "draft",
 "content": "Εδώ γράψε ολόκληρο το σώμα του email απάντησης στα Ελληνικά με βάση την οδηγία του χρήστη."
}

(Αν ο χρήστης κάνει απλή ερώτηση για το email αντί για σύνταξη απάντησης, άλλαξε το intent σε "question" και βάλε την απάντηση στο content).`;
    ...
    try {
        ...
        // Parsing μέσω του ασφαλούς safeJsonParse που φτιάξαμε
        const parsed = safeJsonParse(raw);

        if (!parsed.content || parsed.content.trim() === "..." || parsed.content.trim().length < 5) {
            throw new Error("Το AI επέστρεψε ελλιπές περιεχόμενο (...) λόγω προσωρινής απώλειας σύνδεσης με το Outlook.");
        }
        ...
        // Το σφάλμα δρομολογείται κάτω από το μικρόφωνο χωρίς να χαλάει τα textareas
