...
// 100% Robust JSON parser that strips trailing text/garbage from model responses
function safeJsonParse(rawStr) {
    ...
    if (start === -1) throw new Error("Δεν βρέθηκε αρχικό άγκιστρο '{' στην απάντηση.");
    ...
    // Fallback if structured count fails
...
async function generateSummaryAndAudit() {
    ...
    const prompt = `Είσαι ένα αυτοματοποιημένο backend API. Η απάντησή σου πρέπει να είναι ΑΠΟΚΛΕΙΣΤΙΚΑ ένα έγκυρο αντικείμενο JSON. Απαγορεύεται ρητά να γράψεις εισαγωγικό κείμενο, markdown ή επεξηγήσεις.
...
(Το πεδίο intent μπορεί να είναι είτε "draft" είτε "question")`;
    ...
    try {
        ...
        // Use the bulletproof parser for drafts too
        ...
        // Loop broken! Technical error details pushed under the mic instantly
