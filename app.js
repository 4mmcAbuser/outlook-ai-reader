...
async function generateDraft(instruction, audioObj) {
    ...
    // 🔥 SMART OPTIMIZATION: Περιορίζουμε το μέγεθος στους 10.000 χαρακτήρες
    // για να μην κρασάρει ο server της Google με 503 High Demand σφάλματα.
    ...
    try {
        ...
        const res = await fetch(url, {
            ...
            body: JSON.stringify({
                contents: [{ parts }],
                generationConfig: { 
                    temperature: 0.2 // Χαμηλώνουμε ελαφρώς για μέγιστη σταθερότητα δομής
                }
            })
        });
        ...
        // Parsing μέσω του ασφαλούς safeJsonParse που αντέχει και plain κείμενο
        ...
        // Το σφάλμα απομονώνεται αυστηρά κάτω από το μικρόφωνο
